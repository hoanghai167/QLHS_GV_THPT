...
namespace QL_GV_HS_THPT_DAL
{
    public class SQL_tblGiangday
    {
        ...
        {
            cn.ThucThiCauLenhSQL(@"UPDATE tblGiangday SET Ngayday = '" + et.Ngayday + "', Tietday = '" + et.Tietday + "', MaLop = '" + et.MaLop + "' WHERE MaGV = '" + et.MaGV + "'");
        }
        //Xoa du lieu
        public void delGiangday(EC_tblGiangday et)
        {
            cn.ThucThiCauLenhSQL(@"DELETE FROM tblGiangday WHERE MaGV = '" + et.MaGV + "'");
        }
        //select
        ...
        {
            return cn.getDatatable(@"SELECT magv FROM tblGiaovien");
        }
        public DataTable getMalop()
        {
            return cn.getDatatable(@"SELECT malop FROM tblLop");
        }
    }
}